...
1. topK推荐：猜你喜欢，汇总和用户已经消费国的物品相似的物品，按照汇总后的分数从高到低推荐。思想和user-based相似，用相似度加权汇总。要预测用户u对物品i的评分，就要遍历用户u评分过的所有物品，假如共有m个物品，用每个物品和待计算物品i的相似度乘以用户的评分，加权求和后除以所有这些相似度的总和，得到一个加权平均评分，作为预测分数。在计算过程中只对用户评分过的物品，逐一取出它们的相似物品，不需要计算所有物品。再去掉用户已经消费过的物品，保留分数最高的k个结果进行存储
2. 相关推荐：（推荐某个物品的相似物品）看了又看、买了又买。不需要提前合并计算，当用户访问一个详情页时，直接获取该物品的相似物品推荐  

经典的系统过滤缺点：1）无法实时更新。2）计算相似度没有考虑相似度置新的问题，例如两个物品仅被一个用户喜欢，那么余弦相似度为1，是不可信的，但在计算推荐结果时却影响很大
