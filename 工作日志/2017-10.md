...
## 10.31
#### -15.00
在网上查找资料大多都是小米用小米推送，华为用华为推送，魅族用魅族推送，其他都用友盟或者极光推送，但是小米在其他平台表现也不错，不明白为什么其他平台一定要选其他的SDK。
确定推送方案确定为：华为使用华为推送，其他包括小米都使用小米推送，测试过后如果小米推送在其他平台表现不好，就加上友盟推送或者极光推送。
#### -18.43
> 计划在android创建独立的module，方便其他app导入，不需要在app中配置繁琐的manifest、gradle、res，在module中统一配置，并且暴露设置appId的方法；进一步封装为react native module
> 针对多平台，前端只开启自己平台的推送SDK，后台有两种方式来实现：
> 1. 每个平台都推送一遍
> 2. 前端把设备和用户关联起来传给后台，后台根据用户的设备来使用不同的推送

创建新的demo程序，完成module的创建，小米推送导入成功，可以接收到推送的消息和传输的key-value
> 使用react-native-device-info的getBrand()可以获取到设配的品牌。魅族（Meizu），小米（Xiaomi），华为（？？？）

**小米推送可以传输任何自定义的key-value**
...
> 找到两个分页的组件
[rn-viewpager 439⭐️](https://github.com/zbtang/React-Native-ViewPager)
[react-native-pages 148⭐️](https://github.com/n4kz/react-native-pages)
都很好用。第一个在iOS pop动画时会显示上一页的内容，设置removeClippedSubviews={false}解决。选择了第一个，因为⭐️多，国人出品，而且滚动时可以自动收回键盘

#### -18.19
话题编辑的图片滑动功能完成。每个media管理自己的poi，滑动完成后更新之前页的描述，点击完成后更新当前页的描述（完成按钮不置灰，可能用户确实想清空描述）。如果删除的是最后一张，删除完成后退出页面。

> 滚动时只可以滚动图片，所以高度固定，输入框使用同一个，不可以随着滚动，如果每个media都有一个输入框的话不好获取输入的数据
输入框组件暴露setValue方法，在切换图片后可以更新描述
...
1. CoreFoundation.framework设置为Optional。
2. 所有修改导航栏的代码都只在iOS9及以上执行，iOS8不存在appearanceWhenContainedInInstancesOfClasses方法。`if ([[UIDevice currentDevice].systemVersion floatValue] >= 9.0f)`。结果只是返回按钮的文字大一些。
...
发现了一个图片库[react-native-fast-image](https://github.com/DylanVann/react-native-fast-image)，使用原生图片解决Image的闪烁问题。
![](media/15089846145052.jpg)


~~这个库也使用了SDWebImage，和本地的编译冲突。需要在本地Podfile中注释掉SDWebImage，执行pod install。然后在BuildSetting->Header Search Paths中添加`$(SRCROOT)/../node_modules/react-native-fast-image/ios/Vendor/SDWebImage`，设置为`recursive`，就可以在项目中访问到SD了。~~

#### -14.10
android使用Glide3.\*，和PictureSelector使用的4.*冲突，无法解决，所以android不集成。拷贝iOS源码，同时解决SDWebImage的冲突。在RN创建FastImage组件，android直接返回Image。

android图片选择器PictureSelector同时升级到2.1.3，修复选择视频的一些错误。需要翻墙

iOS使用pod引入FLAnimatedImage，FastImage加载GIF使用
 
#### -18.40
评论点赞数为0时不显示数字

测试个推安卓推送，直接使用官方demo包，和极光一样退出后完全收不到消息。

iOS在FFFastImageView.m中，`[self sd_setImageWithURL: .. : .. : progress: ..]`的progress回调中，去掉targetURL参数，在debug运行失败，加上这个参数，在release编译失败。所以完全去掉progress回调，也用不到。

iOS 11模拟器运行release，按钮点击会闪烁。测试真机没问题，不用管

修复刚打开APP崩溃。因为`[UITabBarItem appearance].badgeColor`不支持iOS10以下的系统，闪退的两个手机系统都是iOS9，现在也用不到原生的tabbar，去掉。另外检查了其他设置nav的方法，都可以。

下载iOS9和iOS8模拟器。兼容低系统，iOS9可以兼容，iOS8未下载完成。
 
## 10.25
### 工作
#### 修复
1. 修复问答详情下拉刷新显示错误。isRefreshing写在了网络请求下面。其他代码类似的地方都对了

2. 圈子列表让搜索框可以和列表一起滚动，否则整个页面可滚动区域太小。
    > 之前没有这样做是存在技术问题，对于两列的listView，containerStyle变了，header的style不好控制。需要在外面套一层ScrollView，ListView设置bounces={false}，跟随外层一起滚动，搜索框和下拉刷新组件都放在ScrollView里
    
3. 圈子列表页，当网络请求发生错误，点击刷新后，错误页面会消失掉，显示正常的等待状态。而不是之前点击后页面没反应，有了数据才反应
    > 需要在loading时设置{error: null}
    > 在reduxHelper中增加了错误的处理方法
    > 圈子列表的reducer也使用reduxHelper
    
4. “0 秒前”替换为“刚刚”逻辑错误。不能使用replace，因为有“10 秒前”，用三目判断==
5. 查找图片闪烁的问题，react-native-fast-image也许能解决，但是和本地的SDWebImage冲突，需要找代替SD的办法。
    
修改友盟的相关设置后，友盟统计已经有正确的ios、android版本了
## 10.24
### 工作
#### 整天
1. 修复话题编辑页，视频无法拖动的bug
    > 因为事件冲突。要在TouchableWithoutFeedback里面加一层flex的View
    
2. 视频播放页加入全屏播放功能；视频封面图的点击区域增大；进入到编辑视频页不自动播放，只是想编辑下描述，马上播放感觉有点烦。
    > 修改react-native-media-kit加入全屏按钮
    > 使用react-native-orientation来控制屏幕旋转，需要把旋转方向全部开启，在app打开后锁定屏幕方向，在按下全屏按钮时再锁定为横向。
    
3. 分享：活动标题修改；分享圈子用户名改为圈子成员的昵称。
4. 话题编辑，编辑视频时，控件栏的宽度永远为屏幕宽度，不是视频宽度，如果视频太高，控件显示不完全。
5. 友盟统计没有android统计信息，忘了在xml中修改umeng的app key
6. android打包半天不成功。卡在Resolving dependencies react-native-device-info_debugPblish，需要连接VPN，并且设置为全局模式。因为这个库引用了google play services


### 总结
* View的pointerEvent根本不好用，无法控制子组件或者父组件的响应。

## 10.23
### 工作
#### 整天
1. Sketch v0.2.7
2. 修复友盟统计版本号显示为build号的错误，iOS友盟统计默认为build号，需要调用setAppVersion()来设置为商店版本号。
3. android：在问题列表回答后崩溃。RNN pop后马上push有bug，pop后需要延迟500ms再push。iOS没问题。而且也不能两次push，因为设定了一次push后间隔1s才能再次push。
4. 修改密码、修改手机号页面pop两次不使用popN，还是使用setTimeout pop一次，为了兼容其他页面。
5. 圈子里使用的reducer，在切换圈子后都马上清除圈子信息
