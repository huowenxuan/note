...
Overlay上弹动画可配置是否弹簧动画、是否支持透明度动画

### -17.0 
# CodePush

## 使用
1. 安装对应版本[CodePush](https://github.com/Microsoft/react-native-code-push)：`npm install react-native-code-push --save`
2. 安装cli：`npm install -g code-push-cli`
3. 注册CodePush：`code-push register`，github登录不了，使用微软账号登录，将生成的tok en复制上去

不管是网站还是命令行都很容易访问失败

```
// 创建iOS
code-push app add turbo-ios ios react-native
// 创建android
code-push app add turbo-android android react-native
// 列出所有app
code-push app ls
// 列出iOS -k同时列出 deployment key
code-push deployment ls turbo-ios -k
// 删除掉无用的Deployment
code-push deployment rm turbo-ios Staging

// 使用react native打包iOS，需要先创建一个bundles文件夹
react-native bundle --platform ios --entry-file index.ios.js --bundle-output ./bundles/main.jsbundle --assets-dest ./bundles
// 打包Android，资源文件比较麻烦
react-native bundle --platform android --entry-file index.android.js --bundle-output ./bundles/main.jsbundle --assets-dest ./bundles 
//  上传到Code Push 
// --mandatory, -m 强制
// --description, --des 更新描述
// --deploymentName, -d 上传到哪个Deployment，默认为Staging
// "<1.0.0"为更新条件，安装的版本小于1.0.0的用户可安装。可以使用1.0.0为针对1.0.0的用户进行安装。更新条件可以覆盖，但是不能重复上传，会检验md5值
code-push release turbo-ios ./bundles "<1.0.0" -d Production -m

// 更推荐使用release-react将两个步骤合为一步，会打包项目根目录中，打包完成后需要手动删除压缩文件
// --targetBinaryVersion, --t 目标版本
code-push release-react turbo-ios ios --t "<1.0.1" -d Production --des "测试"

// 每个上传的版本相当于一个label，查看历史版本
code-push deployment history turbo-ios Production
// 更新某个label
 code-push patch turbo-ios Production -l v1 --mandatory true

// 回滚 --targetRelease, -r 要回滚的label，默认为倒数第二个版本，必须保证两次的版本号一样
code-push rollback turbo-ios Production --targetRelease v2
```

## 测试：
### iOS
在dubug模式下会自动加载node的代码，不会加载CodePush的，所以要设置AppDelegate.m的jsCodeLocation=[CodePush bundleURL];并且把生成的jsbundle拖到iOS项目中，就可以不从node服务加载代码了。**每次测试需要卸载再重新安装app，不能直接重新运行，会认为是回滚的用户，不会再进行更新**
### android
把IP设置为一个不可用的即可
...
### -18.20 
code push打包、上传没问题，研究提醒和安装下载的流程
...
* 华为SDK始终下载不下来，需要在根build.gradle添加maven { url 'http://developer.huawei.com/repo/' }
* 需要兼容多次登录的问题，会造成重复监听
* android MainActivity newIntent中不可使用getIntent()，直接使用intent参数

### -15.56 
讨论活动付款前后的流程。

1. 报名成功后跳转到报名成功页，可以显示报名成功与订单号，可以跳转到订单详情页，也可以返回到活动详情页、
2. 订单详情页可以查看报名信息，可以取消订单，可以去付款。
3. 进入到付款页，就会生成订单号。如果付款成功就按第一步的流程走；如果付款失败，就停留在当前页面，可以手动返回，返回后刷新详情页的状态，显示待付款，点击后进入到订单详情页。
4. 调用付款后，等待app再次返回前台，不使用sdk的支付回调（如果付款后按下home键，再打开app，回调不会执行），而是自己请求这个活动的订单状态

实现：

1. 修改报名管理通过和不通过两个按钮的样式，边框改为背景色

### -17.18 
支付完成。

> 支付宝支付后无法返回app，需要设置scheme为lxq

修改一些规则：

> 因为支付后才会报名，生成报名信息，所以未支付不会有报名信息，所以在我报名的活动里不会存在待支付的状态，而如果加上未付款，管理员也会看到成员的支付状态，分类太多太乱。所以暂定不存在待付款，只有付款成功未付款。对用户不友好的地方只有需要重新写报名信息，可以把报名信息保存到本地
...
计划报名成功页只显示简单的成功字样，类似微信支付成功，下方的有一个按钮可以查看订单详情。

使用Overlay，通过modal动画弹出报名成功页，完成一部分
